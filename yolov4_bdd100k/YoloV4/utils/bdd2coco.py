'''
  Source code is from: https://gist.github.com/zzzxxxttt/4ea0e1bac0293fee518035ae52394746
'''
import os
import json
import argparse
...
parser.add_argument('--bdd_dir', type=str)
cfg = parser.parse_args()

src_val_dir = os.path.join(cfg.bdd_dir, 'labels',
                           'bdd100k_labels_images_val.json')
src_train_dir = os.path.join(
    cfg.bdd_dir, 'labels', 'bdd100k_labels_images_train.json')

os.makedirs(os.path.join(cfg.bdd_dir, 'labels_coco'), exist_ok=True)

dst_val_dir = os.path.join(cfg.bdd_dir, 'labels_coco',
                           'bdd100k_labels_images_val_coco.json')
dst_train_dir = os.path.join(
    cfg.bdd_dir, 'labels_coco', 'bdd100k_labels_images_train_coco.json')
...
def bdd2coco_detection(labeled_images, save_dir):
    attr_dict = {"categories":
                 [
                     ...
                     {"supercategory": "none", "id": 8, "name": "rider"}
                 ]}
    ...
    for i in tqdm(labeled_images):
        ...
        _, idx = os.path.splitext(os.path.basename(file_name))[0].split("-")
        image_id = int(idx, 16)
